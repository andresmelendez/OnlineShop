                            ...
                            <table class="table table-hover text-truncate" id="tablaProductos">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Descuento x caja</th>
                                        <th>Cantidad x volumen</th>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="text" class="form-control" id="cupo" placeholder="" name="cupo"  value="" disabled>
                                </div>
                                <div class="col-md-4">
                                    <label for="Cupo">Total en ordenes <span class="required-label">*</span></label>
                                    <input type="text" class="form-control" id="totalFacturado" placeholder="" name="totalFacturado"  value="" disabled>
                                </div>
                                <div class="col-md-4">
                                    <label for="Cupo">Cupo disponible <span class="required-label">*</span></label>
                                    <input type="text" class="form-control" id="cupoDisponible" placeholder="" name="cupoDisponible"  value="" disabled>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row justify-content-center align-items-center">
                            <div class="col-md-3">
                                ...
                                <h4 class="text-success">Flete: <b id="freight"></b></h4>
...
<script>
                    ...
                    if ($('#datosBuscar').val() !== '') {
                        ...
                        $('.dropdown-item').on('click', function () {
                            ...
                            $.ajax({
                                ...
                                $("#cupo").val(datos.cupo);
                                $("#totalFacturado").val(datos.totalFacturado);
                                $("#cupoDisponible").val(datos.cupoDisponible);
                            });
                        });
                    } else {
                        $('#listaClientes').fadeOut();
    ...
    $(document).on("click", ".btnAgregarProductoPedido", function () {
        $.ajax({
            type: "POST",
            url: "app/pedidos/pedidosAcciones.php",
            data: {codigo: $(this).closest('tr').find('td:eq(0)').text(), cantidad: $(this).closest('tr').find(".cantidadModal").val(), tipoPrecios: $('#tipoPrecios').val(), ciudad: $('#codigoCiudad').val(), accion: 'AgregarProducto'},
        }).done(function (datos) {
            datos = JSON.parse(datos);
    ...
    $(document).on('change', '#tipoPrecios, #codigoCiudad', function () {
        onload(1);
    });
    
    $('#formularioDatos').submit(function (e) {
        e.preventDefault();
        var total = $.trim($('#valorFinal').text().replace('.', '').replace('.', '').replace('.', '').replace('.', '').replace('$', ''));
        ...
        if (document.getElementById('tablaProductosAgregados').rows.length > 1) {
            if(($('#cupo').val() > 0 && parseInt($('#cupoDisponible').val()) > total) || parseInt($('#cupo').val()) === 0 || $('#cupo').val() === '') {
                $.ajax({
                    type: "POST",
                    url: "app/pedidos/pedidosAcciones.php",
                    datatype: "json",
                ...
                alert('No puede pasar el pedido ya que el cupo disponible es menor al total del pedido');
            }
        } else {
    ...
    function actualizar() {
        ...
        var flete = 0;
        if(valorFinal <= 40000) flete = 4000;
        else if(valorFinal >= 40001 && valorFinal <= 60000) flete = 3000;
        else if(valorFinal >= 60001 && valorFinal <= 80000) flete = 2000;
        else if(valorFinal >= 80001 && valorFinal <= 100000) flete = 1000;
        ...
        $("#freight").text(formatoMoneda.format(flete));
        $("#valorFinal").html(formatoMoneda.format(valorFinal + flete));
    }
